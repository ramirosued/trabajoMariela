...
import styles from './multiplicaciones.module.css'; // Importa los estilos CSS
...
export default function Sumas() {
  ...
  const asignarNumero = () => {
    ...
    setResultado(numA*numB); // El resultado es la resta de numA y numB
    setMensaje("");
    setRespuestaUsuario("");
  };
  ...
  const enviarPuntos = async () => {
    const { error } = await supabase
      .from("usuarios")
      .insert([{ nombre, puntos: correctas, tipo: "multiplicacion" }]);
    if (error) {
      ...
      <div className={styles.card}>
        {juegoTerminado ? (
          ...
          <>
            ...
            <div className={styles.problem}>{a} x {b}</div>
